...
class Lorenz63Env(gym.Env):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, render_mode: Optional[str] = None, sigma=10.0, rho=28.0, beta=8/3, integ_steps = 5):
        ...
        low_Obs = np.concatenate((low_Obs, [-30., -30., -4000., -4000., -4000., -60., -60., -10.]))

        high_Obs = np.tile([4000., 4000., 4000., 60., 60., 100.], self.num_steps -1)
        high_Obs = np.concatenate((high_Obs, [30., 30., 4000., 4000., 4000., 60., 60., 100.]))

        self.lowObs  = np.array(low_Obs, dtype=np.float32)
        self.highObs  = np.array(high_Obs, dtype=np.float32)
        self.highAct = np.array([10., 10., 10.], dtype=np.float32)
    ...
    def step(self, action):
        ...
        *_, xR, yR, zR = self.state_ref
        ...
        terminated = bool(self.counterSteps >= 10000000 or np.abs(new_x - new_xR) > 8. or np.abs(new_y - new_yR) > 10. or np.abs(new_z - new_zR) > 15.)
    ...
    def reset(self, *, seed: Optional[int] = 20, options: Optional[dict] = None):
        ...
        low2.append(DEFAULT_X)
        low2.append(DEFAULT_Y)
        low2.append(DEFAULT_Z)
        
        high2.append(DEFAULT_X)
        high2.append(DEFAULT_Y)
        high2.append(DEFAULT_Z)
